...
for random_seed in range(1,11,1):
    ...
    LGBM_Gs_best.booster_.save_model('./Model/LGBM_GS_best_'+str(random_seed)+'.txt')
    ...
    df_result.to_excel('./Model/GridSearch'+'_performance_'+str(random_seed)+'.xlsx')
    X_test['Observed RMC'] = y_test
    X_test['Predicted RMC'] = y_pred
    X_test.to_excel('./Model/Predict_observe'+'_'+str(random_seed)+'.xlsx')
