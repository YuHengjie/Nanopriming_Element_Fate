...
# %%
model_parameters = pd.read_excel("./LightGBM_parameters.xlsx",index_col = 0,)
model_parameters
...
for i in range(1,11,1):
    ...
    sss = StratifiedShuffleSplit(n_splits=1, test_size=test_ratio, random_state=i) 
    ...
    model = lgb.LGBMClassifier(n_jobs=-1,max_cat_to_onehot=9, random_state=42,
                                boosting_type = model_parameters.loc[i,'boosting_type'],
                                num_iterations = model_parameters.loc[i,'num_iterations'],
                                max_bin = model_parameters.loc[i,'max_bin'],
                                min_data_in_leaf = model_parameters.loc[i,'min_data_in_leaf'],
                                min_sum_hessian_in_leaf = model_parameters.loc[i,'min_sum_hessian_in_leaf'],
                                max_depth = model_parameters.loc[i,'max_depth'],
                                num_leaves = model_parameters.loc[i,'num_leaves'],
                                learning_rate = model_parameters.loc[i,'learning_rate'],
                            )
...
print('Acerage absoulte SHAP value for Concentration (mg/L): ', abs(feature_shap_values).mean())

index = feature_name_list.index('BET surface area (m2/g)')
feature_shap_values = shap_values_all[:,index]
print('Acerage absoulte SHAP value for BET surface area (m2/g)s: ', abs(feature_shap_values).mean())

index = feature_name_list.index('Concentration (mg/L)')
feature_shap_values = shap_interaction_values_all[:,index,index]
print('Acerage absoulte SHAP interaction value for Concentration (mg/L): ', abs(feature_shap_values).mean())

index = feature_name_list.index('BET surface area (m2/g)')
feature_shap_values = shap_interaction_values_all[:,index,index]
print('Acerage absoulte SHAP interaction value for BET surface area (m2/g)s: ', abs(feature_shap_values).mean())
...
for feature in ['Concentration (mg/L)', 'TEM size (nm)','Hydrodynamic diameter (nm)', 'Zeta potential (mV)',
    ...
    plt.scatter(X_test_all[feature], global_shap_values, marker='o',s=60,c='#EA8D6C',linewidth=0.4,edgecolors='#FFFFFF')
...
for feature in ['Concentration (mg/L)', 'TEM size (nm)','Hydrodynamic diameter (nm)', 'Zeta potential (mV)',
    ...
    plt.scatter(X_test_all[feature], global_main_effects, marker='o',s=60,c='#EA8D6C',linewidth=0.4,edgecolors='#FFFFFF')
